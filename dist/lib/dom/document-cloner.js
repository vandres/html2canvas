...
*/
    DocumentCloner.prototype.cloneNode = function (node) {
        ...
        if (window && node_parser_1.isElementNode(node) && (node_parser_1.isHTMLElementNode(node) || node_parser_1.isSVGElementNode(node))) {
            ...
            var children = node.shadowRoot ? Array.from(node.shadowRoot.childNodes) : Array.from(node.childNodes);
            for (var i = 0; i < children.length; i++) {
                var child = children[i];
                if (node_parser_1.isSlotElement(child)) {
                    var assignedNodes = child.assignedNodes();
                    if (assignedNodes.length > 0) {
                        // replace the slot element with its assigned nodes, and then process them immediately
                        children.splice.apply(children, [i, 1].concat(Array.from(assignedNodes)));
                        i -= 1;
                        continue;
                    }
                }
